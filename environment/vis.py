...
def save_metrics(history, regions, output_dir="plots", model_name="Test"):
...
def visualize_episode(model=None, model_name="Test", seed=42):
    ...
    region_actions = [3 + i * 4 for i in range(env.num_regions)]  # Action 3, 7, 11, ..., 35 (3000 doses per region)
    ...
    while step < env.max_steps:
        ...
        # Cycle through region-specific dose actions for first 9 steps, then random or model
        ...
        for i, region in enumerate(env.regions):
            ...
            red = int(255 * (1 - coverage))  # Red decreases from 255 to 0
            green = int(255 * coverage)      # Green increases from 0 to 255
            color = (red, green, 0)
...
if __name__ == "__main__":
    ...
    try:
        model = PPO.load("models/best2_2025-08-02_02-15-12.zip")  # Adjust path as needed
        visualize_episode(model, model_name="DQN", seed=42)
    except FileNotFoundError:
        print("DQN model not found. Running with random actions.")
        visualize_episode(model=None, model_name="Random", seed=42)
